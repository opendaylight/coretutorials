...
import org.opendaylight.dhcp.server.RpdLeaseManager;
...
public class DhcpModule extends
    ...
    public DhcpModule(org.opendaylight.controller.config.api.ModuleIdentifier identifier,
            org.opendaylight.controller.config.api.DependencyResolver dependencyResolver,
            DhcpModule oldModule,
            java.lang.AutoCloseable oldInstance) {
        super(identifier, dependencyResolver, oldModule, oldInstance);
    }

    public static void main(String[] args) {
        List<String> networkInterfaces = Arrays.asList("lo", "eth0", "eth1");

        ThreadFactory factory = new DefaultThreadFactory("dhcp-server");
        EventLoopGroup group = new NioEventLoopGroup(0, factory);
       
        startServer(67, networkInterfaces, new RpdLeaseManager(), Collections.<DefaultOption>emptyList(), group);
    }

    @Override
    public void customValidation() {
        JmxAttributeValidationException.checkNotNull(getLeaseManager(), "lease-manager not set", leaseManagerJmxAttribute);

//        JmxAttributeValidationException.checkCondition(!getNetworkInterface().isEmpty(), "no network-interface set",
//                networkInterfaceJmxAttribute);
    ...
    public java.lang.AutoCloseable createInstance() {
        ...
        return startServer(getPort().getValue(), getNetworkInterface(), getLeaseManagerDependency(),
                getDefaultOption(), getWorkerThreadGroupDependency());
    }
