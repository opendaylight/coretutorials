...
parser = argparse.ArgumentParser(description='Sharding Benchmarking')
...
parser.add_argument("--dataitems", type=int, default=[1, 10, 100, 1000, 10000, 100000],
                    nargs="+", help="Number of per-shard data items.")
parser.add_argument("--putspertx", type=int, default=[1, 10, 100, 1000, 10000, 100000], nargs='+',
...
parser.add_argument("--predata", choices=["TRUE", "FALSE"], nargs="+", default=["TRUE", "FALSE"],
                    help="Specifies whether test data should be pre-created before pushing it into the data store."
                         'Default=["FALSE"]')
parser.add_argument("--validata", choices=["TRUE", "FALSE"], nargs="+", default=["FALSE"],
...
def run_test(warmup_runs, test_runs, test_type, datastore, shards, data_items, puts_per_tx, listeners, precreate_data, validate_data):
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        writer.writerow(('Test Type', 'Datastore', 'Shards', 'Data Item', 'Puts per Tx',
                         'Listeners', 'Precreate Data', 'Validate Data'))
        ...
        for ttype in TEST_TYPE:
            ...
            for dtst in DATASTORE:
                ...
                for sd in SHARDS:
                    print 'Shards: %d' % sd
                    writer.writerow(('', '', '%d:' % sd))

                    for di in DATA_ITEMS:
                        print 'Data Item: %d' % di
                        writer.writerow(('', '', '', '%d:' % di))
