...
public abstract class AbstractShardTest implements AutoCloseable {
     ...
     *
     */
    private void createListAnchors(final List<DOMDataTreeIdentifier> dataTreeIdentifiers) {
        final MapNode mapNode = ImmutableMapNodeBuilder
                .create()
                .withNodeIdentifier(new NodeIdentifier(InnerList.QNAME))
                .build();

        final DOMDataTreeProducer producer = dataTreeService.createProducer(dataTreeIdentifiers);
        final DOMDataTreeCursorAwareTransaction tx = producer.createTransaction(false);

        for (final DOMDataTreeIdentifier treeId : dataTreeIdentifiers) {
            final DOMDataTreeWriteCursor cursor = Preconditions.checkNotNull(tx.createCursor(treeId));
        ...
        try {
            ...
            LOG.error("Failed to close producer for {}", dataTreeIdentifiers);
            throw new RuntimeException(e);
        }
    }
