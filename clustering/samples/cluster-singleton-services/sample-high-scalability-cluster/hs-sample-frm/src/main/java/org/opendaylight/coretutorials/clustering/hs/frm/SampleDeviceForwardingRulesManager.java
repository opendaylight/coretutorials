 ...
 *
 */
class SampleDeviceForwardingRulesManager implements ClusteredDataTreeChangeListener<SampleNode>, AutoCloseable {

    private static final Logger LOG = LoggerFactory.getLogger(SampleDeviceForwardingRulesManager.class);

    protected final ConcurrentMap<NodeKey, SampleDeviceForwardingRulesContext> contexts = new ConcurrentHashMap<>();
    ...
    private ListenerRegistration<SampleDeviceForwardingRulesManager> listenerRegistration;

    SampleDeviceForwardingRulesManager(final DataBroker dataBroker, final RpcProviderRegistry rpcProviderRegistry,
    ...
    protected void startDeviceForwardingRulesContext(final InstanceIdentifier<SampleNode> ident, final SampleNode sampleNode) {
        ...
        final FrmDeviceSetupBuilder builder = new FrmDeviceSetupBuilder();
        ...
        contexts.put(nodeKey, new SampleDeviceForwardingRulesContext(builder.build()));
    }

    private ListenableFuture<Void> stopDeviceForwardingRulesContext(final NodeKey nodeKey) {
        final SampleDeviceForwardingRulesContext devContext = contexts.get(nodeKey);
        if (devContext != null) {
