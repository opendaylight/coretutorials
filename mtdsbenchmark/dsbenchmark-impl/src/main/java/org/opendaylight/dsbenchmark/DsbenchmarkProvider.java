...
public class DsbenchmarkProvider implements BindingAwareProvider, DsbenchmarkService, AutoCloseable {
    ...
    public Future<RpcResult<StartTestOutput>> startTest(StartTestInput input) {
        ...
        //Invoke the multi-threaded Test Executor with the configured number of threads
        DsbenchmarkExecutor executor = new DsbenchmarkExecutor(input.getThreadCount());
        StartTestOutput output;

        // Run the tests
        try {
            output = executor.execute(input, dataBroker, domDataBroker);
            this.testsCompleted++;
